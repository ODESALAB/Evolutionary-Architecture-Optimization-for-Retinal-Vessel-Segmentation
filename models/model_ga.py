...
class Model(nn.Module):
    def __init__(self, chromosome, node_boundaries):
        ...
        self.nbr_cell = int(chromosome[-2])
        self.nbr_filters = int(chromosome[-1])
    ...
    def compile(self):
        ...
        for cell_idx in range(self.nbr_cell):
            cell = EncoderCell(self.chromosome, self.node_boundaries, C_in, C_out)
            C_in = C_out
            C_out = C_out * 2
        ...
        for _ in range(self.nbr_cell):
            cell = DecoderCell(self.chromosome, self.node_boundaries, C_in + C_in, C_in)
            self.cells.append(cell)
            C_in = C_in // 2
            C_out = C_out // 2
